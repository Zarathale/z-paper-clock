...
| M0.6 — Authoring/QA preview tool (`preview.html`) | ⏹ Frozen mid-flight 2026-05-10. 29 merged PRs: v1a foundation, cut-layer, texture-flip, back-face-mirror, perf, thickness fix, axle rotation, v1b face graph + hinge tree, source-of-truth loader, face-graph diagnostics, cut-trim, panels-aware parser, fold-step + closure-attach, multi-piece scene, inferred-connections audit, assembled-pose load + save, Bench/Cluster foundation, transform capture, marks-lookup, bridge button, Cluster mode, snap-to-connection-point, bridge save, guided assembly stepper, grouped fold sliders, derive-one closure folds. Architecture decision **closed 2026-05-07** (DECISIONS #4, Option B). |
| Panels-first SVG authoring | ⏹ Frozen at **32 exported SVGs across 41 piece folders**, 28 pieces in the connection graph — anchor / pendulum / bob complete (065-072, 093a/093b, 094-100, 110), plus 001, 002, 037, 038, 041, 044, 045, 047, 058, 059, 090, 113. A partial motor-wheel set (033-049) has Affinity sources without exports. Connection graph carries 51 declared cross-piece edges, 30 resolving.  Only four pieces have JSON sidecars (065, 066, 067, 069). Conventions stable post-`claude-work/DECISIONS.md` #7; `LAYER-CONVENTIONS.md` is the live reference. |
...
6. **Verification Checklist** — numbered post-implementation checks Code runs against the finished work (file exists, lints pass, output is what we expected, version constants match if version-numbered). **For `preview.html` or any browser-rendered file, split explicitly into two tiers:** *(a) Code-side* — syntax checks (extract the `<script>` body to a temp `.js` file and run `node --check` on that — `node --check preview.html` fails on Node 24+ with `ERR_UNKNOWN_FILE_EXTENSION`; ~5 lines of Python does the extract), ripgrep for undefined symbols, file-existence checks, Python script output validation; *(b) Browser (Alan post-merge)* — interactive checks that require a live browser. **Code must NOT start an HTTP server as a verification step.** Port conflicts with any server Alan already has running, combined with the worktree path mismatch (Code works in `.claude/worktrees/<name>/`, not the repo root), make this approach unreliable. If Code cannot reach a running server that it knows is already started, it falls back to Code-side static checks only and documents which browser-side items remain for Alan. Code-side checks are the commit gate; browser checks are the post-merge gate.
7. **What NOT to Change** — named exclusions. Stops Code from drifting into out-of-scope refactors.
8. **Manual tests** (optional) — small table or list of pre-/post-conditions Zarathale runs after merge against the local checkout. Include when the change is observable end-to-end. For `preview.html` changes this section is **not optional** — it is the browser-verification tier from item 6 and must be present.
...
│   ├── viewer/                             empty stub — NEVER populated. DECISIONS #4 (closed 2026-05-07) puts the real viewer at claude-work/viewer/ at M3
...
Cowork sessions are **not** branch-isolated — Cowork edits the user's mounted folder directly, and does not create or work on git branches the way Code mode does (`claude/<topic>` branches and `.claude/worktrees/` are a Code-mode artifact). What does happen is more boring and more annoying: the bash sandbox sees your selected folder via a mac→Linux mount layer, and **that layer can lag behind your real-time edits within a single session**. Two observable symptoms:

1. **Stale mtime even when content has changed.** Cowork reads `NNN.svg`, sees the old content, the file's mtime claims the same `2026-05-05 23:13` it had ten minutes ago — but you actually saved an updated version mid-session. Cowork's read silently captures the new content on a *later* re-read but the mtime never moves. So if Cowork uses mtime as a freshness oracle, it will report "nothing's changed" when something has.
2. **Edits not propagating at all within the session.** You re-export `NNN.svg` from Affinity; Cowork sees the file unchanged in size, content, and mtime. A `find -mmin -<recent>` from the sandbox shows no recent writes. The edit hasn't crossed the mount layer yet.

**Reliable signal: a fresh session always sees current state.** If you commit locally (or just save) and then start a new Cowork session, the new session mounts fresh and sees everything correctly. The friction is purely *within-session*, not across sessions.

**Cross-session is fine; within-session, don't trust mtime.** When the user has just authored or re-exported and asks Cowork to look, Cowork should:

1. **State exactly what it's reading before drawing conclusions.** "I see `090.svg` at 872 bytes, mtime 2026-05-05 16:34 — does that match what you see on your mac?" gives the user the chance to catch the divergence in the moment.
2. **Triangulate beyond mtime.** Compute `sha256sum`, file size, and a content fingerprint (e.g., the count or names of specific layer ids the user just authored). Mtime is unreliable across the mount; content sha and size are more honest.
3. **Use `git status` as the synchronization point.** If your mac sees `M work/pieces/NNN/NNN.svg` and Cowork's sandbox-side `git status work/pieces/NNN/` shows no changes, the mount has not propagated. Git diffs against object content, so it's robust to the mtime weirdness; it's the cleanest cross-side oracle for "is the file you saved the same as the file Cowork sees?"
4. **Don't conflate "Cowork sees the old version" with "the user didn't save."** When Cowork's read disagrees with what the user reports, the default conclusion should be "the mount is stale," not "the user is mistaken." If a re-read after a few seconds doesn't reconcile, ask the user to commit (or just `git status`) on their mac side and report what they see — that disambiguates.

**Mitigations when the divergence has already happened in-session:**

- **Quick check the user can run on their mac:** `git status work/pieces/NNN/` and `stat ~/Documents/GitHub/z-paper-clock/work/pieces/NNN/NNN.svg`. Paste both lines into Cowork. Cowork runs the equivalent in the sandbox; if outputs disagree, the mount is stale.
- **Force-flush attempt from the sandbox:** `sync && ls -la work/pieces/NNN/` then re-read. Sometimes nudges the kernel to refresh attribute cache; not reliable but cheap to try.
- **The hard reset:** if Cowork is genuinely blind to a file the user has saved, **commit locally and start a new Cowork session**. The new session's mount is correct on the first read. This is the working escape hatch and it works every time.

**Authoring workflow stays as-is.** Saving Affinity exports directly into `work/pieces/NNN/NNN.svg` ("alongside" the `.af`) is fine; the friction is on Cowork's reading side, not the user's authoring side. The one lightweight habit that helps: after a re-export you want Cowork to see, run `git status work/pieces/NNN/` on your mac and paste the output. That single line cuts through the ambiguity faster than any sandbox-side stat dance.
...
- **Per-element ids inside `<g id="marks">`** (settled 2026-05-03; same-piece closure form added 2026-05-07). **Landing markers** are how a piece marks the panels that receive a tab. Two valid forms: (1) **Cross-piece** — `landing-<tab-letter><piece-number>` reads as "this is the landing on this piece for tab `<letter>` from piece `<number>`." `landing-c70` (tab `c` from piece 70 lands here); `landing-h65`; letter-variant pieces format as `landing-<tab><piece-bare-with-letter>` e.g. `landing-a92a`. (2) **Same-piece closure** — `landing-<panel-id>` where `<panel-id>` is bare letters naming a panel on this same piece (a tab wraps around to land here on the same piece, e.g. on a long strip that closes into a cylinder). Examples: `landing-d`, `landing-taba`, `landing-tabb` (closures on 069), `landing-aa` (closure on 001), `landing-tabaa` (066 closure). Authored as small ellipse / circle / path elements. Tab letter is the lowercase letter from the printed plate; piece number is the bare numeric id (no zero-padding) to match the in-print notation. Multiple landings per piece are common. The marker's centroid is the landing point; for now landings are points, not regions (extend to bounding shapes if a piece needs it). Other element types inside `<g id="marks">` (construction lines, registration marks, dotted alignment guides, anything else from the print) need no id and are rendered as decals only. Consumed today by: `preview.html` (categorizes cross-piece vs `landing-same-piece` for closure-attach), the audit script's `landing-marker-id-format` check (regex accepts both forms), and the connection-graph builder. The cross-piece pairing (tab `c` on piece 70 ⟷ `landing-c70` on piece 71) is the connection-graph primitive M4 will read; same-piece closures pair `tab-<letter>` ⟷ `landing-<panel-id>` within a single piece's adjacency graph.
...
- **Code sessions: do not start an HTTP server to verify `preview.html`.** Code runs in a worktree (`.claude/worktrees/<name>/`); starting a server there creates a path mismatch with any server Alan already has running at the repo root, and the port may not be bindable. For syntax validation, extract the `<script>` body to a temp `.js` file and run `node --check` on that (`node --check preview.html` fails on Node 24+ with `ERR_UNKNOWN_FILE_EXTENSION`; ~5 lines of Python does the extract). Use ripgrep for symbol checks; document the remaining browser-side checks in the Manual tests section for Alan to run post-merge. See "Orchestration Prompt Format" item 6 for the full two-tier verification protocol.
...
*Last updated: 2026-05-07 (evening) — **same-piece closure landings formally accepted; new "Cowork mount sync caveats" section added.** Two passes in one closing session. (1) The marks-layer `Per-element ids` entry was lagging behind LAYER-CONVENTIONS.md — only documented the cross-piece form `landing-<tab-letter><piece-number>`. Expanded to document both forms (cross-piece + same-piece closure `landing-<panel-id>`) with examples (`landing-d`, `landing-taba`, `landing-tabb`, `landing-aa`, `landing-tabaa`); audit_state.py's `LANDING_ID_RE` widened from `^landing-[a-z][0-9]+[a-z]?$` to `^landing-[a-z]+(?:[0-9]+[a-z]?)?$` so closure-form ids stop tripping the format check; check description updated to `"landing-* ids match landing-<letter><piece-number>[variant] OR landing-<panel-id> format"`. Verified against 5 pieces with landings (001/069/071/100/113) — all pass post-fix. (2) New section **Cowork mount sync caveats** between "Mac + git locks" and "Post-merge cleanup" — captures the within-session friction where the cowork-side mount lags behind real-time mac-side authoring, mtime can be stale even when content has changed, and re-exports may not propagate at all within a session. Documents the diagnostic pattern (state what's being read; triangulate via sha + size + content fingerprint; use `git status` as the synchronization point, not mtime), the working escape hatch (commit locally + start a fresh cowork session — the new mount always reads correctly on first read), and the principle that the friction is on cowork's reading side, not the user's authoring side (Affinity "save alongside" workflow stays as-is). Also: stray `Document.svg` + `main.svg` files (Affinity untitled-export artifacts) deleted from `work/pieces/097/` and `work/pieces/100/`. See `sessions/2026-05-07-1930_cowork_marks-layer-recheck-and-mount-sync.md`.*

*Earlier 2026-05-06 (post-PR-19 review) — **shipped CODE_PROMPTs now move to `_archive/code-prompts/`.** Convention flip: orchestration prompts at repo root only while in flight (`status: draft | in-development | ready-for-code`); on ship, they move to `_archive/code-prompts/` as the decision record. Same folder also hosts killed-without-shipping prompts (`status: archived` with `archived_reason:`). Driver: 21 stale shipped prompts had accumulated at root; `_archive/code-prompts/` already existed with two archived drafts. Edits in this pass: intro paragraph + "Orchestration Prompt Format" section (both Naming and After-ship paragraphs) + "End-of-Code-session" step #6 + Repo Structure tree + File Naming Conventions Orchestration-prompts row all reflect the new rule. Repo Structure tree expanded to include `claude-work/`, `_archive/code-prompts/`, `LAYER-CONVENTIONS.md`, `PROJECT-STATE.md`, `ROADMAP.md`, `WORKPLAN.md`, `preview.html`, `tag-pieces.html`, `work/SPEC-REGIONS.md`, `work/state.json`, `work/piece_characters_v2.yaml`, `work/expected_layers.yaml`, `.claude/` — all of which had landed since the last tree refresh. Footer shipped/superseded prompt references (e.g. `CODE_PROMPT_filesystem-restructure.md`, `CODE_PROMPT_preview-html-cut-layer.md`) are bare filenames in historical context and stay as-is — bare filenames resolve correctly against the archive folder when used as references.*
