...
> **⏹ FROZEN 2026-08-22.** Work stopped 2026-05-10 at commit `5a6b49b`. The M0.6 table below was **caught up at the freeze** with the seventeen ships that had accumulated after task 0.6.14 (rows 0.6.15–0.6.31) so it no longer stops mid-stream. The milestone index near the top was **not** rewritten and its hour estimates are stale — M0.6's "~6 actual" covers a fraction of what shipped. Milestone states at freeze: M0.5 capture complete but the pipeline reshape never done; M0.6 frozen mid-flight; M1 archived against retired scans; M2 onward never started.
...
| 0.6.13 | **Architecture decision: graduate preview.html into `work/viewer/`, keep separate, or skip `work/viewer/` entirely?** | **done** | 1.0 | ~1.0 | Cowork | Resolved-decision row in this roadmap + SPEC §"Path forward" updated | **Closed 2026-05-07 as Option B** (`claude-work/DECISIONS.md` #4): `preview.html` stays permanently as the authoring/QA tool; a fresh TypeScript + Vite + three.js viewer is built at `claude-work/viewer/` when M3 is imminent; no code shared. `work/viewer/` was always an empty stub and will never be populated. See `sessions/2026-05-07-1700_cowork_architecture-and-attach-convention.md`. |
...
| 0.6.15 | Face-graph diagnostic harness (PR #13) | done | — | — | Code | Console diagnostics for region/adjacency debugging | 2026-05-04. |
| 0.6.16 | Cut-trim: cut regions with `fold.start`/`fold.end` rather than authored endpoints (PR #14) | done | — | — | Code | 066: 27 regions, 17/17 markers, 13 residual orphans | 2026-05-04. DECISIONS #5. Best cut-line-first ever got; the residual drove the panels-first pivot. |
| 0.6.17 | Panels-aware parser + hinge forest (PR #15) | done | — | — | Code | `parsePanelsLayer`, `parsePanelsFirstFolds`, `buildHingeTree`, dispatch banner | 2026-05-05. The cut step disappears; authoring is authoritative. |
| 0.6.18 | Fold-step ordinal prefix + same-piece closure attach (PR #16) | done | — | — | Code | `<step>-fold-<a>-<b>` in slider labels; `attach-same-piece` classification | 2026-05-05. DECISIONS #9. |
| 0.6.19 | Multi-piece scene mode (PR #17) | done | — | — | Code | `loadScene`, `renderSceneMulti`, pivot-cluster co-location | 2026-05-06. Anchor cluster renders as one scene. |
| 0.6.20 | Inferred cross-piece connections in the audit (PR #18) | done | — | — | Code | `build_assembly_graph.py` reads `connections.inferred[]`; every edge tagged `provenance` | 2026-05-06. DECISIONS #10. Audit-side, not preview-side. |
| 0.6.21 | Assembled-pose load + save (PR #19) | done | — | — | Code | `assembled.folds` read at load; "Save assembled pose" emits a snippet | 2026-05-06. DECISIONS #11. |
| 0.6.22 | Bench/Cluster foundation + cutouts as Shape holes (PR #20) | done | — | — | Code | Closes 0.6.10. TransformControls, camera lock, RGB gizmos, worktable | 2026-05-09. DECISIONS #13, PR A. |
| 0.6.23 | Bench-mode transform capture (PR #21) | done | — | — | Code | `computePieceOrigin`, 6-slider Transform panel, `assembled.transform` read+write | 2026-05-09. DECISIONS #13, PR B. |
| 0.6.24 | Parser consults `marks` for cross-piece lookup (PR #22) | done | — | — | Code | 8-step lookup chain; `*.svg` globbing brings 093a/093b into the graph | 2026-05-10. DECISIONS #12. Graph 24 → 30 resolving edges. |
| 0.6.25 | "→ Claude" bridge button (PR #23) | done | — | — | Code | POSTs parsed piece state to `localhost:7777`; `preview_bridge.py` writes it to `claude-work/state/` | 2026-05-10. Replaced the copy-paste console-dump workflow. |
| 0.6.26 | Cluster mode multi-piece authoring (PR #24) | done | — | — | Code | Per-piece transform editing in a cluster, distance readouts, per-piece save | 2026-05-10. DECISIONS #13, PR C. |
| 0.6.27 | Snap-to-connection-point (PR #25) | done | — | — | Code | Graph-driven partner highlighting, median-translation snap-all, rigid-group lock | 2026-05-10. |
| 0.6.28 | Direct sidecar save via the bridge (PR #26) | done | — | — | Code | `POST /save` merges `{piece, assembled}` into the sidecar; copy modal as offline fallback | 2026-05-10. Eliminated hand-merging JSON. |
| 0.6.29 | Guided assembly stepper (PR #27) | done | — | — | Code | 12-task guided panel driven by a 68-step §II.C sequence with a `depends_on` DAG | 2026-05-10. Paired with `claude-work/INSTRUCTIONS-ANNOTATED.md`. |
| 0.6.30 | Grouped fold sliders (PR #28) | done | — | — | Code | `classifyFoldId()`; pane-strip Σ readout, tab-flap/closure master sub-slider | 2026-05-10. Driving case: 066's 22 folds across four groups. |
| 0.6.31 | Derive-one closure fold + ±360° Σ readout (PR #29) | done | — | — | Code | Lock ⊗ derives `fold-tabaa-pane7` from the `landing-tabaa` mark | 2026-05-10. **The last feature shipped before the freeze.** |
